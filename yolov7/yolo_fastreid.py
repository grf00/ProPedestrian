...
from fast_reid_master.fastreid.config import get_cfg
from fast_reid_master.fastreid.utils.logger import setup_logger
from fast_reid_master.fastreid.utils.file_io import PathManager
from fast_reid_master.predictor import FeatureExtractionDemo
from fast_reid_master.fastreid.data import build_reid_test_loader
from fast_reid_master.fastreid.data.transforms.build import build_transforms
from yolov7.fast_reid_master.fastreid.data.common import CommDataset
...
from lama.lama_pre2 import LaMa       #单张图像修复
...
#################################################################################################################################
    ...
    # --------------- lama 初始化-------------------
    device_la = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    lama_model = LaMa(device_la)
    print("lama加载完成")
    #################################################################################################################################
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                if gallery_img:
                    ...
                    #################################修复模块############
                    #masked = (im0 * (1 - mask[..., np.newaxis] / 255.0)).astype(np.uint8)
                    lama_image = lama_model(masked, mask)
                    #################################隐藏模块############
                    cover_image_pil = Image.fromarray(cv2.cvtColor(lama_image, cv2.COLOR_BGR2RGB))#lama是naddary，改成PIL可读取的方式
                    cover_image = preprocess(cover_image_pil).unsqueeze(0).cuda()
                    secret_image_pil = Image.fromarray(cv2.cvtColor(gallery_img[index[0]], cv2.COLOR_BGR2RGB))  # 转换为 PIL 图像
                    secret_image = preprocess(secret_image_pil).unsqueeze(0).cuda()  # 加载并转为批量形式
                    # # 隐写
                    steg_image = steg.steganography(cover_image, secret_image)#
                    steg_image_1 = torchvision.utils.make_grid(steg_image)#steg_image是四维tensor，torchvision.utils.make_grid可以转三维
                    ndarr = steg_image_1.mul(255).add_(0.5).clamp_(0, 255).permute(1, 2, 0).to('cpu',torch.uint8).numpy()#还原图像像素，原本进行归一化
                    im = Image.fromarray(ndarr)
                    steg_image_transf = T.Compose([T.CenterCrop((h0, w0))])#剪裁至原来大小
                    steg_image_pil = steg_image_transf(im)
                    steg_image_pil.save(opt.lama_pic + p.name[0:-4] + "_lama_st.png")
            ...
            if save_img:
                if dataset.mode == 'image':
                    ...
                    print(f" The steganography with the result is saved in: {opt.lama_pic}")
